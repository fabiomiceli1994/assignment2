...
As pointed out in $\S$(\ref{subsec:the-present-case}), BVPs can be solved through numerical methods for linear algebra, if one manages to represent the problem in a convenient matrix form. An appropriate member function has been implemented for this task: \emph{MatrixBuild} constructs matrix $A$ of Eqn.\eqref{eqn:matrix-form} with the entries given by \eqref{eqn:matrix}.\\
Such a construction takes places within the function \emph{ADR$\_$Test}, that performs the tests required by the assignment instructions and outputs the results on different .txt files. For clarity reasons and in order to make the program more ``user-friendly",  each time a simulation is performed, a suitable success-message, containing the different parameters of the respective simulation, is printed runtime on the terminal.\\ 
Finally, the analitical solutions at each point of $\Omega_h$ are obtained by means of the function \emph{An$\_$sol}, which simply compute Eqn.\eqref{eqn:AD-sol} or Eqn.\eqref{eqn:DR-sol} at the given set of points.\\
